...
REPORT_MANIFEST: tuple[ReportManifestEntry, ...] = (
    ...
    ReportManifestEntry(
        ...
        data_period="not available until local macro context CSV is provided",
        source="TCMB/EVDS, TUIK and Federal Reserve planned via local CSV workflow",
        sample_size="0 valid macro rows; 5 required indicators missing",
        assumptions="requires observed period, publication timestamp, download timestamp and source URL per row",
        limitations="RSS macro alias context does not replace numeric macro records",
    ),
    ReportManifestEntry(
